...
It looks very similar to what we already know. The only difference is, that we provide the `say` method with a user ID and a driver name. I like to create a Laravel command which holds this code. It gives more freedom on how I will trigger the message later. Like with a cronjob.
