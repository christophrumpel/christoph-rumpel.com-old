...
Normally, this is the message flow: The user sends something, the bot replies. This is the same with the `get_started` button in Facebook. Here the button triggers the first "message", to which the bot replies. But with our newsletter bot, we want the bot to make the first step. In BotMan terms this is called `originating messages`. This is an example from the BotMan documentation.
...
First the botman instance is resolved from the service container. Then we grab all the subscribed users from the database, in order to loop over them. We now send each user a message. The content is now totally up to you. You can send just one message or multiple ones with different templates as well. For this example is one message is enough. You probably also notices that we can personalize the messages.

<div class="blognote"><strong>Note:</strong> It is important to only provide helpful content to the user. We don't want to create a nother spam channel here.</div>
