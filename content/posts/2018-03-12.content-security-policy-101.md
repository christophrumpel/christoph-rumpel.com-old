...
```text
report-uri https://christophrumpel.report-uri.com/r/d/csp/enforce;base-uri 'self';connect-src 'self';default-src 'self';form-action 'self' christoph-rumpel.com christoph-rumpel.test christoph-rumpel.us5.list-manage.com;img-src 'self' * 'unsafe-inline' data:;media-src 'self';object-src 'none';script-src 'self' 'nonce-HylQ7PzuRX8PRZaCxzFUJ6CpbFBpP70C' christoph-rumpel.com christoph-rumpel.test fonts.googleapis.com *.google-analytics.com *.googletagmanager.com *.facebook.com *.facebook.net;style-src 'self' christoph-rumpel.com christoph-rumpel.test data: fonts.googleapis.com *.facebook.com *.facebook.net 'unsafe-inline';font-src fonts.gstatic.com data:;frame-src *.facebook.com
```
...
This is because I use Google Analytics on my blog and there are scrips required to make it work. So we add a `script-src` directive and allow scripts from the site's origin and `googletagmanager.com.`
...
<div class="blognote"><strong>Note:</strong> Unfortunately, the report-uri directive is depricated. It still works in some browsers, but there will be a new one. Read more <a href="https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/report-uri">here</a>.</div>
...
<br /><br />
**PS**: If you know a way to avoid the `unsafe-inline` for styles or got any other question, just contact me on [Twitter](https://twitter.com/christophrumpel).
<br /><br />
**Update**: Seems like I wasn't finished there after this article. A day later I had to [deal with CSP and response caching](https://christoph-rumpel.com/2018/03/laravel-response-caching-and-csp) and [CSP, hashing and Turbolinks](https://christoph-rumpel.com/2018/03/content-security-policy-hash-algorithm-and-turbolinks).
