...
<br /><br />
The chatbot provides a quiz with all kinds of questions about the Laravel framework. Every question comes with up to three possible replies. You need to pick the right one to collect points and make it to the highscore. But besides the ranks, the quiz is about having a good time. Enjoy the questions and see what you know about Laravel. Have fun!
...
Every question has a `text` and `points`. The user would get these points if the answer was correct. 
...
As you can see, the code is quite straightforward. It is just a big array with all the data for questions and answers. Don't forget to import the namespaces for the `Answer` and `Question` class. Inside the `database/seeds/DatabaseSeeder.php` I add this new seeder to the `run` method.
...
Voila, we have our questions and answers in the database!

## QuizConversation

Now that everything is setup, we can go on and implement the chatbot logic for showing the questions to the user. I already know that I want to use an own conversation for this code. Create a `app/Conversation/QuizConversation.php` file and fill it with this base code.
...
```
Then, before showing a question, we check if there is one left. After every question we will remove it. When no one is left, we show the user the quiz result.
...
Also, we add a `showResult` method, which is called when there is no more question left. When you start the conversation again, you should be able to see all the questions. Each click of an answer will result in the next question, and after the last one, you should get the finish message. Try it out yourself.
...
Then we add a method for storing highscore entries. I use the `updateOrCreate` method to prevent duplicate entries. Every user should only have on entry. With every new try, it will get updated. The unique field for these entries is the `chat_id` and not, like normally, the `id`. The `updateOrCreate` gets this info as the first parameter. Also, note how I increase the `tries` field if the entry is not a new one. For this, the `wasRecentlyCreated` method comes handy.
...
For the highscore, only the top users will be shown.
...
We take the users with the most points and add a new rank field to every entry.
...
<br /><br />
To trigger this conversation, we need another listener. Besides the `highscore` keyword I also add `/highscore` which will become Telegram command later.
...
Type one of the keywords to the bot to see what happens. It will tell you that it is still empty.

<img class="blogimage" alt="Screenshot showing empty highscore" src="/images/blog/laravelquiz_highscore_empty.png" />

For testing purposes, we can manually add a few highscore entries to the table. I will use Sequel Pro to do that. Now we should get some highscore entries back. Notice the users with the same points will share the rank.
...
Since the GDPR it is essential to give a user the possibility to delete its stored data. I want to handle that in a new `app/Conversations/PrivacyConversation.php`. First we make sure we have stored this current user. If yes, we ask him if he really wants to delete his highscore entry. 
...
This is an excellent time to add the Telegram commands to the bot. You need to use the `BotFather` to achieve that. Follow his instructions to edit the bot and add the commands. You always have to fill all the commands at once.
